...
public class Runigram {
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		int rows = image.length;
		int cols = image[0].length;
		Color[][] fImage = new Color[rows][cols];
		for (int i = 0; i < rows; i++){
			for (int j = 0; j < cols; j++){
				fImage[rows - i - 1][j] = image[i][j];
			}
		}
		return fImage;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		...
		int rows = image.length;
		int cols = image[0].length;
		Color[][] grayed = new Color[rows][cols];
		for (int i = 0; i < rows; i++){
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		//// Replace the following statement with your code
		double w = (double)image[0].length / height;
		double h = (double)image.length / width;
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		*/
		for (int i = 0; i <= n; i++) {
			imageOut = blend(source, target, ((double)(n - i) / n));
			display(imageOut);
